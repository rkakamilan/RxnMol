...
RDLogger.DisableLog("rdApp.*")  # disable error message.
...
def tight_rm_branch(_smi_l, _smi_r):
    ...
    while n_branch > 0:  # over opened-branch
        ...
        if n > 0.5:  # 추가
            ...
            while not branch_gate:  # Ring 부분을 피해서 자름
                if j == limit_:
                    # Debug
                    new_smi = _smi_l + _smi_r
                    # logger.info("tight_rm_branch Error Type1")
                    # logger.info("Return: %s", new_smi)
                    logger.debug(f"tight_rm_branch Error Type1: {_smi_l}+{_smi_r} = {new_smi}")

                    return new_smi
                    # raise ValueError

                rmin, rmax = _smi_l_open_branch[-1] + 1, _smi_r_open_branch[-1] + q

                if rmin >= rmax:
                    new_smi = _smi_l + _smi_r
                    logger.debug(f"tight_rm_branch Error Type2: {_smi_l}+{_smi_r} = {new_smi}")

                    return new_smi

                b = np.random.randint(rmin, rmax)
                j += 1
    ...
    while n_branch < 0:  # over closed-branch
        ...
        if n > 0.5:
            ...
            j = 0
            while not branch_gate:  # Ring 부분을 피해서 자름
                rmin, rmax = _smi_l_close_branch[-1] + 1, _smi_r_close_branch[0] + q

                if rmin >= rmax:
                    new_smi = _smi_l + _smi_r
                    logger.debug(f"tight_rm_branch Error Type3: {_smi_l}+{_smi_r} = {new_smi}")
                ...
                if j == limit_:
                    new_smi = _smi_l + _smi_r
                    logger.debug(f"tight_rm_branch Error Type4: {_smi_l}+{_smi_r} = {new_smi}")
                    return new_smi
                    # raise ValueError
            n_branch += 1
            if b < len(_smi_l):
...
def replace_atom(_smi):
    ...
    #                    C, N, P,  O,  P, F, Cl, Br
    replace_atom_list = [6, 7, 15, 8, 16, 9, 17, 35]
    # replace_atom_list = [6, 7, 15, 8, 16, 9, 17, 35]
    ...
    while not gate_:
        ...
        try:
            ...
            # raise ValueError(f"{max_len = }, {max_trial = }, {p = } Failed to replace_atom: {_smi}")
            rnd_atom = 0
        ...
        try:
            ...
            if mw.GetAtomWithIdx(rnd_atom).GetIsAromatic():
                ...
                if valence == 4:
                    ...
                    mw.ReplaceAtom(
                        rnd_atom, Chem.Atom(replace_atom_list[np.random.randint(1, 3)])
                    )
                elif valence == 2:
                    mw.ReplaceAtom(
                    ...
                    mw.ReplaceAtom(
                        rnd_atom, Chem.Atom(replace_atom_list[np.random.randint(0, 8)])
                    )
        except:
            pass
        p += 1
...
def delete_atom(_smi):
    ...
    while True:
        ...
        try:
            ...
            # raise ValueError(f"{max_len = }, {max_trial = }, {p = } Failed to delete_atom: {_smi}")
            rnd_insert = 0
    ...
    Chem.Kekulize(mol_)
    _new_smi = Chem.MolToSmiles(mol_, kekuleSmiles=True)

    # print("delete_atom>", _new_smi)
...
def add_atom(_smi):
    ...
    list_atom = ["C", "N", "P", "O", "S", "Cl", "Br", "F"]
    # avoid_ring = [i for i, j in enumerate(_smi) if j.islower() or j.isdigit()]
    # rnd_insert = np.random.randint(max_len)

    max_len = len(_smi)
    if max_len < 0:
        logger.debug("Failed to add_atom: %s", _smi)
    mol_ = False
    _new_smi = None
    ...
    while True:
        ...
        try:
            ...
            # raise ValueError(f"{max_len = }, {max_trial = }, {p = } Failed to add_atom: {_smi}")
            rnd_insert = 0
        _new_smi = (
    ...
    # print("add_atom>", _new_smi)
    return _new_smi, mol_


def crossover_two_smiles(smi1, smi2):
    """
    Input: two SMILES strings, smi1, smi2
    Return: SMILES string

    Perform a crossover operation between two SMILES strings.

    """
    l_smi = None
    r_smi = None
    try:
        l_smi = prepare_rigid_crossover(smi1, "L", False)
        r_smi = prepare_rigid_crossover(smi2, "R", False)
    ...
    while not mol:
        ...
        k += 1
        if k == max_trial:
            logger.debug("Failed to Crossover: %s, %s", smi1, smi2)
...
if __name__ == "__main__":
    ...
    smiles = []
    cids = []
    with open("sampled_ZINC_0.001.smi") as inf:
        a = 0
        for l in inf:
            if a == 0:
                a += 1
            ...
            if mol:
                ...
                logger.debug("Errorneous smiles: %s", smile)
                continue
            smiles.append(
    ...
    for idx in range(niter):
        ...
        if idx % 100 == 0:
            logger.info(f"{new_smi}\t{n_valid}\t{n_fail}")

        if mol:
            n_valid += 1
            logger.info(f"# {idx}, new_smi: {new_smi}")
            # print(f"{new_smi}\t{n_valid}\t{n_fail}")
        ...
        if idx % 100000 == 0:
            logger.info(f"Success rate: {n_valid / (n_valid + n_fail) * 100}%")

    logger.info(f"Final Success rate: {n_valid / (n_valid + n_fail) * 100}%")
